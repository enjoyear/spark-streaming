...
```bash
kubectl apply -f ./k8s-spark/spark-operator/wc-jar/word-count-configmap.yaml
```
...
```bash
kubectl apply -f ./k8s-spark/spark-operator/wc-jar/word-count-dockerimage.yaml
```
...
docker build -t spark-wordcount:v2 -f k8s-spark/spark-operator/wc-jar/Dockerfile .
